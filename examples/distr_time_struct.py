...
# import sys
stdlib_dir = dirname(os.__file__)
__path__.append(join(__path__,"../"))
...
def extract_pnms():
    ...
    path_to_result = "/home/andrey/PHD/Kerogen/data/Kerogen/tmp/result_time_depend_struct/i2p/"
    # Opening JSON file
    f = open(config_path)
...
def build_distributions(path_to_pnms:str)->None:
    ...
    for step, file in sorted_lfiles[::2]:
        psd_params, tld_params, radiuses, throat_lengths = extract_weibull_psd(
            join(path_to_pnms, file[:-10]), 1e9, 1
        )
